...
await axiosPrivate.post(`http://localhost:4000/getprofileexperience/${applicants.usernumber}`,{number:applicants.usernumber},
{
...
const getuserprofile = async()=>{
    await axiosPrivate.post(`http://localhost:4000/internprofile`,{number:applicants.usernumber},
    {
     withCredentials:true
    }).then((response) => {
...
const getuserskills =async()=>{
  await axiosPrivate.post(`http://localhost:4000/getuserskills/${applicants.usernumber}`,{number:applicants.usernumber},
{
 withCredentials:true
}).then((response) => {
   console.log(response.data.recordset);
...
await axiosPrivate.post(`http://localhost:4000/getprofileeducation/${applicants.usernumber}`,{ number:applicants.usernumber},
{
 withCredentials:true
}).then((response) => {
   console.log(response.data.recordset);
    ...
    <div className='flex h-[450px] w-5/6 mx-auto '>
      <div className='flex flex-col mt-4 box-border w-full h-[450px] border-2 border-black'>
        ...
        <div class="h-1/6 flex items-center justify-center">
          ...
          <div className='flex w-1/3  mx-2'>
          <button className='w-full py-1  font-serif border-2 border-gray-400 hover:border-none rounded-lg hover:text-white hover:bg-blue-800'>
           Hire
           </button>
          </div>
          <div className='flex w-1/3  mx-2'>
